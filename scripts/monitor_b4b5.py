"""Flash and capture one Stage B4B.5 ESP32-P4 qualification run."""
...
def capture(port: str, timeout: float, output: pathlib.Path) -> int:
    lines: list[str] = []
    ...
    return 0 if re.search(r"B4B\.5 RESULT:\s*PASS", text) else 2
...
def main() -> int:
    ...
    if not args.no_flash:
        flash(args.port)
        time.sleep(.25)
    return capture(args.port, args.timeout, args.output)
